...
contract Voyage3CRVMetaPoolIntegration is VoyageLPIntegration {
    ...
    address internal constant LP_3CRV_TOKEN =
    ...
    function calcRewardAmount(
        address _stablecoin,
        uint _3CRVLPTokens
    ) external view onlyRewardTokens(_stablecoin) returns (uint) {
        return
            ICurve3CRVBasePool(CRV_BASE_POOL).calc_withdraw_one_coin(
                _3CRVLPTokens,
    ...
    function _farmEmergencyWithdrawal(
        ...
        uint basePoolLPTokens = ICurve3CRVPool(CRV_META_POOL)
            .remove_liquidity_one_coin(_metaPoolLPTokens, 1, _minAmounts[0]);

        uint[3] memory minTokenAmounts = [
            _minAmounts[1],
            _minAmounts[2],
            _minAmounts[3]
        ];
        emergencyWithdrawnTokens = _removeBasePoolLiquidity(
            basePoolLPTokens,
            minTokenAmounts
        );
    }
    ...
    function _handleFee(uint _minAmount) internal override {
        ...
        uint usdtFee = _removeBasePoolLiquidityOneCoin(
            fee,
            USDT_TOKEN,
            _minAmount
        );
    ...
    function _removeBasePoolLiquidityOneCoin(
        ...
        ICurve3CRVBasePool(CRV_BASE_POOL).remove_liquidity_one_coin(
            _lpTokens,
            _getBasePoolStablecoinIndex(_stablecoin),
            _minAmount
        );
    ...
    function _removeBasePoolLiquidity(
        uint _lpTokens,
        uint[3] memory _minAmounts
    ) internal returns (uint[3] memory received) {
        ...
        ICurve3CRVBasePool(CRV_BASE_POOL).remove_liquidity(
            _lpTokens,
            _minAmounts
        );
