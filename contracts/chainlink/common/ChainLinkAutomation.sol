    ...
    AutomationCompatibleInterface
{
    ...
    modifier onlyKeeperRegistry() {
        require(
            _msgSender() == keeperRegistry,
            'ChainLinkAutomation: Only the keeper registry can call this function'
        );
        _;
    }
    ...
    constructor(address _keeperRegistry) {
        require(
            _keeperRegistry != address(0),
            'ChainLinkAutomation: _keeperRegistry cannot be the zero address'
        );
    ...
    function _checkUpkeep(
        bytes calldata _checkData
    ) internal virtual returns (bool upkeepNeeded, bytes memory performData);
