...
contract OracleManaged is Ownable {
    ...
    modifier onlyOracle() {
        require(
            _msgSender() == _oracle,
            'OracleManaged: caller is not the Oracle'
        );
        _;
    }
    ...
    modifier onlyOwnerOrOracle() {
        require(
            _msgSender() == owner() || _msgSender() == _oracle,
            'OracleManaged: Only the owner or oracle can call this function'
        );
        _;
    }
    ...
    function _setOracle(address _newOracle) internal {
        require(
            ...
            'OracleManaged: _newOracle cannot be the zero address'
        );
