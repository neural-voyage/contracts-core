...
contract OperatingSystemStaking is OracleManaged {
    ...
    function enableDepositing() external onlyOwner {
        require(
            !isDepositingEnabled,
            'OperatingSystemStaking: Depositing is already enabled'
        );
    ...
    function disableDepositing() external onlyOwner {
        require(
            isDepositingEnabled,
            'OperatingSystemStaking: Depositing is already disabled'
        );
    ...
    function setMinimumDistribution(
        uint _minimumDistribution
    ) external onlyOwner {
        require(
            _minimumDistribution >= BASE_DISTRIBUTION_AMOUNT,
            'OperatingSystemStaking: _minimumDistribution must be greater than the base'
        );
    ...
    function deposit(uint _amount) external {
        require(
            isDepositingEnabled,
            'OperatingSystemStaking: Depositing is not allowed at this time'
        );
        require(
            _amount >= MINIMUM_STAKE,
            'OperatingSystemStaking: Amount must be greater than 1 OS'
        );
    ...
    function withdraw(uint _amount) external {
        ...
        require(
            _amount > 0,
            'OperatingSystemStaking: Amount to withdraw must be greater than zero'
        );
        require(
            _amount <= user.staked,
            'OperatingSystemStaking: Amount exceeds staked'
        );
        /// @dev merge rewards prior to updating their staked balance because their rewards are dependant on their stake
        _mergeRewards();
        _users[_msgSender()].staked -= _amount;
    ...
    function claimRewards() external {
        ...
        require(
            heldRewards > 0,
            'OperatingSystemStaking: No rewards available to claim'
        );
    ...
    function _getCalculatedRewards(address _user) private view returns (uint) {
        ...
        return
            (balance * (currentRewardFactor - _users[_user].rewardFactor)) /
