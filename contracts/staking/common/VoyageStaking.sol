...
abstract contract VoyageStaking is Ownable, ReentrancyGuard {
    ...
    modifier onlyValidLockPeriod(uint _lockUpPeriodType) {
        require(_lockUpPeriodType <= 3, 'VoyageStaking: Invalid lock period');
        _;
    }
    ...
    modifier onlyValidStakeId(address _user, uint _stakeId) {
        ...
        require(
            ...
            'VoyageStaking: Invalid stake ID'
        );
        _;
    }

    modifier onlyIfInitialized() {
        require(initialized, 'VoyageStaking: Contract not initialized');
        _;
    }
    ...
    constructor(
        ...
        require(
            ...
            'VoyageStaking: _stakingToken cannot be the zero address'
        );
        require(
            _rewardToken != address(0),
            'VoyageStaking: _rewardToken cannot be the zero address'
        );
    ...
    function initialize() external onlyOwner {
        require(!initialized, 'VoyageStaking: Already initialized');
        initialized = true;
    ...
    function deposit(
        ...
        nonReentrant
        onlyIfInitialized
        onlyValidLockPeriod(_lockUpPeriodType)
    {
        require(
            isDepositingEnabled,
            'VoyageStaking: Depositing is not allowed at this time'
        );
        require(
            _amount >= minimumDeposit && _amount > 0,
            'VoyageStaking: Amount is not valid'
        );
        uint forecastRewards = _calculateReward(_amount, _lockUpPeriodType);
        require(
            forecastRewards <= totalAvailable,
            'VoyageStaking: Forecast rewards exceeds available rewards'
        );
    ...
    function withdraw(
        uint _stakeId,
        uint _amount
    ) external nonReentrant onlyValidStakeId(_msgSender(), _stakeId) {
        ...
        require(
            ...
            'VoyageStaking: You cannot unstake before the lock up period'
        );
        require(_amount > 0, 'VoyageStaking: You cannot withdraw zero tokens');
        require(
            stake.amountWithdrawn < stake.amount,
            'VoyageStaking: Already withdrawn full amount'
        );
        require(
            _amount <= stake.amount - stake.amountWithdrawn,
            'VoyageStaking: Insufficient stake balance'
        );

        stake.amountWithdrawn += _amount;

        if (stake.claimedRewards < stake.allocatedRewards) {
            uint unclaimedRewards = stake.allocatedRewards -
    ...
    function claimRewards(
        ...
        require(
            stake.claimedRewards < stake.allocatedRewards,
            'VoyageStaking: You have already claimed your rewards'
        );
        uint period = getLockUpPeriod(stake.lockUpPeriodType);
        uint rewards;
        if (block.timestamp >= stake.stakedAt + period) {
            ...
            uint rewardsToDate = (stake.allocatedRewards * timeDifference) /
                period;
    ...
    function getUserStakes(
        ...
        require(
            ...
            'VoyageStaking: Start index must be less than or equal to end index'
        );
        require(
            _startIndex < totalUserStakes,
            'VoyageStaking: Invalid start index'
        );
        require(
            _endIndex < totalUserStakes,
            'VoyageStaking: Invalid end index'
        );
    ...
    function getAPR(
        uint _lockUpPeriodType
    ) public view onlyValidLockPeriod(_lockUpPeriodType) returns (uint apr) {
    ...
    function getLockUpPeriod(
        uint _lockUpPeriodType
    ) public pure onlyValidLockPeriod(_lockUpPeriodType) returns (uint time) {
        if (_lockUpPeriodType == 0) {
            time = 30 days;
        } else if (_lockUpPeriodType == 1) {
            time = 90 days;
    ...
    function _calculateReward(
        uint _amount,
        uint _lockPeriodType
    ) internal view virtual returns (uint reward);
}
