...
/// @title Voyage vesting contract
contract VoyageVesting is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    IERC20 public voyage;
    ...
    modifier onlyValidVestmentId(address _user, uint _vestmentId) {
        ...
        require(
            numberOfVestments > 0,
            'VoyageVesting: No vestments found for user'
        );
        require(
            _vestmentId < numberOfVestments,
            'VoyageVesting: Invalid vestment ID'
        );
        _;
    }
    ...
    function importData(
        ...
        require(
            !_hasVestingStarted(),
            'VoyageVesting: Cannot import data after vesting has started'
        );
        require(
            _addresses.length == _amounts.length,
            'VoyageVesting: _addresses and _amounts list lengths do not match'
        );
        require(
            _tgePercent < TGE_PERCENT_DENOMINATOR,
            'VoyageVesting: _tgePercent must be less than 100'
        );
        uint total;
        for (uint i; i < _addresses.length; i++) {
            ...
            if (_tgePercent > 0) {
                tgeAmount =
                    (_amounts[i] * _tgePercent) /
                    TGE_PERCENT_DENOMINATOR;
            }
    ...
    /// @param _voyage Voyage token address
    function startVestingPeriod(IERC20 _voyage) external onlyOwner {
        require(
            address(_voyage) != address(0),
            'VoyageVesting: _voyage cannot be the zero address'
        );
        require(
            !_hasVestingStarted(),
            'VoyageVesting: Vesting period has already started'
        );
        require(totalAmount > 0, 'VoyageVesting: No data has been configured');
        voyage = _voyage;
        vestingStartedAt = block.timestamp;
        voyage.safeTransferFrom(_msgSender(), address(this), totalAmount);
        emit VestingPeriodStarted();
    }

    /// @notice Withdraw Voyage tokens
    function withdraw(
        uint _vestmentId
    ) external nonReentrant onlyValidVestmentId(_msgSender(), _vestmentId) {
        require(
            _hasVestingStarted(),
            'VoyageVesting: Unlock period has not started'
        );
        User storage user = _users[_msgSender()];
        Vestment storage vestment = user.vestments[_vestmentId];
        bool unclaimedTGE = !vestment.isTGEAmountWithdrawn &&
            vestment.tgeAmount > 0;
        bool unclaimedVestment = vestment.withdrawn < vestment.amount;
        require(
            unclaimedTGE || unclaimedVestment,
            'VoyageVesting: You have already withdrawn the total amount'
        );
        uint amount;
        if (unclaimedTGE) {
        ...
        voyage.safeTransfer(_msgSender(), amount);
    ...
    function getVestment(
        address _user,
        uint _vestmentId
    )
        external
        view
        onlyValidVestmentId(_user, _vestmentId)
        returns (Vestment memory)
    {
    ...
    function _availableToWithdraw(
        ...
        if (block.timestamp >= endDate) {
            ...
            amount =
                ((_vestment.amount * timeDifference) / _vestment.period) -
                _vestment.withdrawn;
        }
    }
