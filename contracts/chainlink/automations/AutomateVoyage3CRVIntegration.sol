...
/// @title ChainLink automation for Voyage 3CRV integrations
contract AutomateVoyage3CRVIntegration is ChainLinkAutomation {
    using SafeERC20 for IERC20;

    Voyage3CRVMetaPoolIntegration public immutable integration;
    ...
    event RewardThresholdUpdated(
        uint metaToken3CRVRewardThreshold,
        uint crv3CRVRewardThreshold
    );
    event SlippageUpdated(
        uint metaTokenTo3CRVSlippage,
        uint crvTo3CRVSlippage,
        uint feeSwapSlippage
    );

    /// @param _integration Voyage 3CRV meta pool integration address
    ...
    constructor(
        ...
        require(
            ...
            'AutomateVoyage3CRVIntegration: _integration cannot be the zero address'
        );
        integration = Voyage3CRVMetaPoolIntegration(_integration);
    }
    ...
    function set3CRVRewardThreshold(
        uint _metaToken3CRVRewardThreshold,
        uint _crv3CRVRewardThreshold
    ) external onlyOwner {
        metaToken3CRVRewardThreshold = _metaToken3CRVRewardThreshold;
        crv3CRVRewardThreshold = _crv3CRVRewardThreshold;
        emit RewardThresholdUpdated(
            metaToken3CRVRewardThreshold,
            crv3CRVRewardThreshold
        );
    }
    ...
    */
    function setSlippage(
        uint _metaTokenTo3CRVSlippage,
        uint _crvTo3CRVSlippage,
        uint _feeSwapSlippage
    ) external onlyOwner {
        require(
            _metaTokenTo3CRVSlippage <= MAX_SLIPPAGE,
            'AutomateVoyage3CRVIntegration: _metaTokenTo3CRVSlippage cannot exceed the maximum slippage'
        );
        require(
            _crvTo3CRVSlippage <= MAX_SLIPPAGE,
            'AutomateVoyage3CRVIntegration: _crvTo3CRVSlippage cannot exceed the maximum slippage'
        );
        require(
            _feeSwapSlippage <= MAX_SLIPPAGE,
            'AutomateVoyage3CRVIntegration: _feeSwapSlippage cannot exceed the maximum slippage'
        );
    ...
    function _performUpkeep(bytes calldata _performData) internal override {
        (
            ...
            uint minFeeAmount
        ) = abi.decode(_performData, (bool, bool, uint[10], uint));
        if (harvestNeeded) {
    ...
    function _checkUpkeep(
        ...
        if (integration.oracle() == address(this)) {
            IVoyageFeeHandler feeHandler = IVoyageFeeHandler(
            ...
            if (metaTokenRewardsIn3CRV >= metaToken3CRVRewardThreshold) {
                upkeepNeeded = true;
                harvestNeeded = true;
                minHarvestAmounts[0] = _calculateMinAmount(
                    metaTokenRewardsIn3CRV,
                    metaTokenTo3CRVSlippage
                );
            ...
            if (crvRewardsIn3CRV >= crv3CRVRewardThreshold) {
                upkeepNeeded = true;
                harvestNeeded = true;
                minHarvestAmounts[1] = _calculateMinAmount(
                    crvRewardsIn3CRV,
                    crvTo3CRVSlippage
                );
            ...
            if (totalFee > 0) {
                if (integration.isShutdown()) {
                    ...
                    */
                    minFeeAmount = _calculateMinAmount(
                        integration.calcRewardAmount(
                            feeHandler.usdt(),
                            totalFee
                        ),
                        feeSwapSlippage
    ...
    function _getCRVRewardsIn3CRV() internal returns (uint expected) {
        ...
        (bool success, bytes memory returnData) = address(curveSwap).call(
            abi.encodePacked(
                ...
                abi.encode(
                    integration.getCRVTo3CRVRoute(),
                    integration.getCRVTo3CRVSwapParams(),
                    rewards
                )
            )
        );
    ...
    function _getMetaTokenRewards() internal view returns (uint) {
        ICurve3CRVGauge gauge = ICurve3CRVGauge(integration.CRV_GAUGE());
        uint balance = IERC20(integration.META_TOKEN()).balanceOf(
            address(integration)
        );
        uint reward = gauge.claimable_reward(
            address(integration),
    ...
    function _getMetaTokenRewardsIn3CRV()
        internal
        view
        returns (uint expected)
    {
        uint rewards = _getMetaTokenRewards();
        /// @dev get_dy reverts if rewards = 0
        if (rewards > 0) {
            expected = ICurve3CRVPool(integration.CRV_META_POOL()).get_dy(
                0,
                1,
                rewards
            );
