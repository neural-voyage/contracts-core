...
import 'contracts/integrations/common/VoyageETHLPIntegration.sol';

/// @title Voyage 3CRV - Meta pool integration
contract VoyageCurveETHPoolIntegration is VoyageETHLPIntegration {
    using SafeERC20 for IERC20;

    /// @dev Meta pool specific addresses
    address public immutable CRV_POOL;
    address public immutable CRV_GAUGE;

    /// @dev Common Curve contracts
    address public constant CRV_SWAP =
        0x81C46fECa27B31F3ADC2b91eE4be9717d1cd3DD7;
    address internal constant CRV_DEPOSIT_ZAP =
        0xA79828DF1850E8a3A3064576f380D90aECDD3359;
    address internal constant CRV_MINTER =
    ...
    /// @param _crvPool Meta pool address
    ...
    constructor(
        ...
        address _oracle,
        address _feeHandler
    ) VoyageETHLPIntegration(_oracle, _feeHandler) {
        require(
            _crvPool != address(0),
            'Voyage3CRVMetaPoolIntegration: _metaPool cannot be the zero address'
        );
        require(
            _gauge != address(0),
            'Voyage3CRVMetaPoolIntegration: _gauge cannot be the zero address'
        );
        CRV_POOL = _crvPool;
        CRV_GAUGE = _gauge;
        META_TOKEN = ICurve3CRVPool(CRV_POOL).coins(0);
        _queryUnderlyings();
    }
    ...
    function calcWithdrawalAmount(
        address _stablecoin,
        uint _metaPoolLPTokens
    ) external view onlyWithdrawalTokens(_stablecoin) returns (uint) {
        return
            ICurve3CRVDepositZap(CRV_DEPOSIT_ZAP).calc_withdraw_one_coin(
                CRV_POOL,
                _metaPoolLPTokens,
                _getStablecoinIndex(_stablecoin)
            );
    }
    ...
    function getCRVTo3CRVRoute() public view returns (address[9] memory route) {
        route[0] = CRV_TOKEN;
    ...
    function getCRVTo3CRVSwapParams()
        public
        pure
        returns (uint[3][4] memory swapParams)
    {
        swapParams[0][0] = 1;
        swapParams[0][2] = 3;
        swapParams[1][0] = 2;
    ...
    function _farmDeposit(
        ...
        uint[4] memory amounts = _constructAmounts(WETH_TOKEN, _amount);
        IERC20(WETH_TOKEN).safeIncreaseAllowance(CRV_DEPOSIT_ZAP, _amount);
        lpTokens = ICurve3CRVDepositZap(CRV_DEPOSIT_ZAP).add_liquidity(
            CRV_POOL,
            amounts,
            _minLPAmount
        );
        IERC20(CRV_POOL).safeIncreaseAllowance(CRV_GAUGE, lpTokens);
    ...
    function _farmWithdrawal(
        ...
        IERC20(CRV_POOL).safeIncreaseAllowance(CRV_DEPOSIT_ZAP, _lpTokens);

        received = ICurve3CRVDepositZap(CRV_DEPOSIT_ZAP)
            .remove_liquidity_one_coin(
                CRV_POOL,
                _lpTokens,
                _getStablecoinIndex(WETH_TOKEN),
                _minAmount
            );
    ...
    function _farmHarvest(
        ...
        if (_minAmounts[0] > 0) {
            ...
            rewards += _exchangeMetaTokenFor3CRV(
                metaTokenBalance,
                _minAmounts[0]
            );
        }
    ...
    function _farmEmergencyWithdrawal(
        ...
        ICurve3CRVBasePool(CRV_POOL).remove_liquidity_one_coin(
            _metaPoolLPTokens,
            _getStablecoinIndex(WETH_TOKEN),
            _minAmounts[0]
        );
    ...
    function _withdrawAfterShutdown() internal override {
        _mergeRewards();
        for (uint i; i < 3; i++) {
            address stablecoin = ICurve3CRVBasePool(CRV_POOL).coins(i);
    ...
    function _claimRewards(
        ...
        require(
            heldRewards > 0,
            'Voyage3CRVMetaPoolIntegration: No rewards available'
        );

        users[_msgSender()].heldRewards = 0;

        rewards = _removeBasePoolLiquidityOneCoin(
            heldRewards,
            _stablecoin,
            _minAmount
        );
    ...
    function _handleFee(uint _minAmount) internal override {
        ...
        uint wethFee = _removeBasePoolLiquidityOneCoin(
            fee,
            WETH_TOKEN,
            _minAmount
        );

        IERC20(WETH_TOKEN).safeTransfer(feeHandler, wethFee);
    }
    ...
    function _constructAmounts(
        address _stablecoin,
        uint _amount
    ) internal view returns (uint[4] memory amounts) {
    ...
    function _removeBasePoolLiquidityOneCoin(
        uint _lpTokens,
        address _stablecoin,
        uint _minAmount
    ) internal returns (uint received) {
        uint stablecoinBalanceBefore = IERC20(_stablecoin).balanceOf(
            address(this)
        );
        ICurve3CRVBasePool(CRV_POOL).remove_liquidity_one_coin(
            _lpTokens,
            _getBasePoolStablecoinIndex(_stablecoin),
            _minAmount
        );
        uint stablecoinBalanceAfter = IERC20(_stablecoin).balanceOf(
            address(this)
        );
    ...
    function _exchangeMetaTokenFor3CRV(
        uint _amount,
        uint _minAmount
    ) internal returns (uint received) {
        IERC20(META_TOKEN).safeIncreaseAllowance(CRV_POOL, _amount);
        received = ICurve3CRVPool(CRV_POOL).exchange(0, 1, _amount, _minAmount);
    }
    ...
    function _getBasePoolStablecoinIndex(
        address _stablecoin
    ) internal view returns (int128) {
